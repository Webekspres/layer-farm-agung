...
import { createVaccineProgramSchema } from "@/features/health/schemas/vaccine-program";
import { createVaccineProgram } from "@/features/health/services/create-vaccine-program";

export type VaccineProgramFormState = {
  error?: string;
  success?: boolean;
};
...
export async function createVaccineProgramAction(
  ...
  const result = await createVaccineProgram(tenantId, parsed.data);
  if (!result.ok) {
